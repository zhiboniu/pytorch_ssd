...
def jaccard(box_a, box_b):
    ...
    area_a = ((box_a[:, 2] - box_a[:, 0]) *
              (box_a[:, 3] - box_a[:, 1])).unsqueeze(1).expand_as(inter)  # [A,B] a(num_objects, 1)
    area_b = ((box_b[:, 2] - box_b[:, 0]) *
              (box_b[:, 3] - box_b[:, 1])).unsqueeze(0).expand_as(inter)  # [A,B] b(1, num_priors)
...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx,
    ...
    index = torch.arange(best_prior_idx.size(0)).long()
    if use_gpu:
        index = index.cuda(gpu_id)
    best_truth_idx[best_prior_idx[index]] = index
...
def match_rep(threshold, predicts, truths, priors, variances, labels, loc_t, loc_g, conf_t, idx):
    ...
    conf = labels[best_truth_idx] + 1  # Shape: [num_priors], suppose class label from 0, and add background
    conf[best_truth_overlap < threshold] = 0  # label as background

    loc = encode(matches, priors, variances)  # [tx, ty, tw, th] = matches & priors

    loc_t[idx] = loc  # [num_priors,4] encoded offsets to learn
    conf_t[idx] = conf  # [num_priors] top class label for each prior

    # jaccard index, predict with gt, not anchor with gt
    predicts = decode(predicts, priors, variances)  # predict_G[x, y, w, h] = predicts & priors
    overlaps = jaccard(
        truths,
        predicts
    )
    #     for i in range(best_truth_idx.size(0)):   #num_priors
